...
describe "rake sitemap:create", type: :system do
  ...
  it "generates a sitemap for every tenant" do
    ...
    Setting["url"] = "http://consul.dev"
    Setting["process.debates"] = false
    Setting["process.proposals"] = false

    Apartment::Tenant.switch("debates") do
      Setting["url"] = "http://debates.consul.dev"
      Setting["process.budgets"] = false
      Setting["process.proposals"] = false
    end

    Apartment::Tenant.switch("proposals") do
      Setting["url"] = "http://proposals.consul.dev"
      Setting["process.budgets"] = false
      Setting["process.debates"] = false
    end
    ...
    debates_sitemap = File.read(Rails.root.join("public", "sitemaps", "debates", "sitemap.xml"))
    proposals_sitemap = File.read(Rails.root.join("public", "sitemaps", "proposals", "sitemap.xml"))
    ...
    expect(debates_sitemap).to have_content debates_path(host: "debates.consul.dev")
    expect(debates_sitemap).not_to have_content budgets_url
    expect(debates_sitemap).not_to have_content proposals_path

    expect(proposals_sitemap).to have_content proposals_path(host: "proposals.consul.dev")
    expect(proposals_sitemap).not_to have_content budgets_url
