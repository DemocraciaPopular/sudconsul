class CreateSharedExtensionsSchema < ActiveRecord::Migration[6.0]
  def up
    ...
    puts full_warning_message if warning_messages.any?
  end
  ...
  def down
    ...
    execute "DROP SCHEMA IF EXISTS #{extensions_schema};"
  end
  ...
  private
    ...
    def full_warning_message
      message = <<~WARNING
        ---------------------- Multitenancy Warning ----------------------
          Multitenancy is a feature that allows to manage multiple
          ...
          If you aren't going to use multitenancy, you can safely ignore
          this warning.
